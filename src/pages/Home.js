...
// ─── FOUNDER PHOTO ────────────────────────────────────────────────────
// 1. Save a photo of Dr. A.Q. Khan as:  src/asset/image/aqkhan.jpg
// 2. Uncomment the import below:
// import aqkhanImg from "./asset/image/aqkhan.jpg";
// 3. Change `null` to `aqkhanImg` in the line after that.
const founderPhoto = null; // ← replace null with aqkhanImg after adding the file
...
.hm-aside-num{font-family:'Playfair Display',serif;font-size:24px;font-weight:700;color:var(--gold);min-width:90px;}
...
.hm-org-tab{padding:11px 28px;border-radius:100px;font-size:13px;font-weight:600;letter-spacing:.5px;color:rgba(255,255,255,.5);background:none;border:none;cursor:pointer;transition:all .25s;font-family:'DM Sans',sans-serif;white-space:nowrap;}
.hm-org-tab.active{background:var(--gold);color:var(--navy);}
@media(max-width:620px){.hm-org-tabs{flex-direction:column;border-radius:16px;width:100%;}.hm-org-tab{border-radius:12px;text-align:center;}}
...
.hm-board-card{background:rgba(255,255,255,.05);border:1px solid rgba(201,168,76,.18);border-radius:16px;padding:28px 20px;text-align:center;transition:transform .3s,background .3s,border-color .3s;}
.hm-board-card:hover{transform:translateY(-6px);background:rgba(201,168,76,.07);border-color:rgba(201,168,76,.4);}
.hm-board-avatar{width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,var(--blue),#0d2550);border:2px solid var(--gold);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-family:'Playfair Display',serif;font-size:22px;font-weight:700;color:var(--gold);}
...
const boardDirectors = [
  { name:"Ma'am Shireen",        initials:'MC' },
  { name:'Dr. Najaf Ali Mohsin', initials:'NA' },
  { name:'Rashid Minhas',        initials:'RM' },
  { name:'Mehboob Sahib',        initials:'MS' },
];

const boardGovernors = [
  { name:"Ma'am Hafsa",         initials:'MH' },
  { name:"Ma'am Anum",          initials:'MA' },
  { name:'Prof. Aqeel Gul',    initials:'AE' },
  { name:'Prof. Amjad Qureshi', initials:'AQ' },
];

// ── Exported so FacultyPage can import from a single source ──
export const facultyData = [
  { id:1,  name:'Ms. Sadia',       subject:'Accounting and Finance Department',          experience:'8+ Years',  qual:'MS Finance',                  gender:'female', photo:null },
  { id:2,  name:'Ms. Faiza',       subject:'Humanities',        experience:'10+ Years',  qual:'Bachelor of Arts',             gender:'female', photo:null },
  ...
  { id:5,  name:'Sir Sikandar',    subject:'Academic Incharge', experience:'2+ Years',  qual:'BS Artificial Intelligence',               gender:'male',   photo:null },
  { id:6,  name:'Sir Abdul Rafay', subject:'Accounts Incharge',          experience:'2+ Years',  qual:'BS Law, Law College Rawalpindi',           gender:'male',   photo:null },
  { id:7,  name:'Ms. Batool',      subject:'Biology',             experience:'7+ Years',  qual:'M.sc Micro Biology',             gender:'female', photo:null },
  ...
  { id:10, name:'Mrs. Nazim',      subject:'TarjamatulQuran',      experience:'2+ Years', qual:'MA Islamiat, International Islamic University',    gender:'female', photo:null },
      ...
      <div className="hm">
        ...
        <nav className="hm-nav">
          <div className="hm-nav-inner">
            <span style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:'var(--gold)',letterSpacing:.5}}>Dr. A.Q. Khan College</span>
            ...
            <AnimatePresence mode="wait">
              {orgTab===0 && (
                <motion.div key="founder" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} transition={{duration:.45}} className="hm-founder-slide">
                  <div className="hm-founder-photo-wrap">
                    {founderImg
                      ...
                      : <div className="hm-founder-emblem">⚛️</div>
                    }
                ...
                <motion.div key="ceo" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} transition={{duration:.45}} className="hm-founder-slide">
                  <div className="hm-ceo-photo-wrap">
                    {ceoImg1
                  ...
                  <div className="hm-founder-role">CEO of Dr. A.Q. Khan's Institution</div>
                </motion.div>
              )}
              {orgTab===2 && (
                <motion.div key="directors" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} transition={{duration:.45}}>
                  <div className="hm-board-grid">
                    {boardDirectors.map((d,i)=>(
                      <motion.div key={i} className="hm-board-card" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:i*.08}}>
                        <div className="hm-board-avatar">{d.initials}</div>
                        <div className="hm-board-name">{d.name}</div>
                        <div className="hm-board-title">Board of Directors</div>
                      </motion.div>
                    ))}
                  </div>
                </motion.div>
                ...
                <motion.div key="governors" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} transition={{duration:.45}}>
                  <div className="hm-board-grid">
                    {boardGovernors.map((g,i)=>(
                      <motion.div key={i} className="hm-board-card" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:i*.08}}>
                        <div className="hm-board-avatar">{g.initials}</div>
                        <div className="hm-board-name">{g.name}</div>
                        <div className="hm-board-title">Board of Governors</div>
                      </motion.div>
                    ))}
                  </div>
                </motion.div>
        ...
        <section className="hm-section hm-faculty-section">
          <div className="hm-container">
            ...
            <div className="hm-faculty-grid">
              {facultyData.map((f,i)=>(
                <Link key={f.id} to={`/faculty#${f.id}`} className="hm-fac-card hm-fade" style={{transitionDelay:`${i*.06}s`}}>
                  <div className="hm-fac-head">
                    <div className="hm-fac-avatar">
                      {f.name.split(' ').filter(w=>w.length>1).map(w=>w[0]).slice(0,2).join('')}
                    </div>
